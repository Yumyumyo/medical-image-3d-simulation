...
class ObjectListWidget(QtCore.QObject):  # 繼承 QObject
    """
    右上「已匯入檔案」列表（Original Objects List）。

    欄位：
        0: 顯示   (checkbox → SceneObject.visible)
        1: 選取   (checkbox → SceneObject.selected，用來決定 multi-cut)
        2: 物件名稱 (文字，可改名)
        3: 透明度 (QSlider 0~100 → opacity 0.0~1.0)
        4: 設定   (QPushButton "..."，點擊後呼叫 on_settings_requested(obj_id))

    不直接碰 VTK：
        - 只透過 ObjectPropertyManager 更新資料
        - 透過 Object3DManager 同步 actor 的外觀
    """
    ...
    def eventFilter(self, obj, event):
        """事件過濾器：監視樹狀元件的事件"""
        if obj is self.tree and event.type() == QtCore.QEvent.Show:
            # 顯示時重新設定寬度
            QtCore.QTimer.singleShot(10, self._force_column_widths)
    ...
    # ======================================================================
    # 新增物件（original SceneObject）
    # ======================================================================

    def add_object(self, obj_id: int) -> None:
        """
        在列表中新增一列對應 obj_id 的 SceneObject（kind="original"）。
        """
    ...
    # ======================================================================
    # QTreeWidgetItem → checkbox 變動（顯示 / 選取 / 名稱）
    # ======================================================================
    ...
    # ======================================================================
    # 設定按鈕（...）點擊：交給 main_window 的 callback
    # ======================================================================
    ...
    def find_item(self, obj_id: int) -> Optional[QtWidgets.QTreeWidgetItem]:
        """
        由 obj_id 在列表中找到對應的 QTreeWidgetItem。
        """
    ...
    def remove_object(self, obj_id: int) -> None:
        """
        從列表中移除某個物件（通常用在刪除 result 時，original 通常只隱藏不刪）。
        """
    ...
    def refresh_from_manager(self, obj_id: int) -> None:
        """
        依照 ObjectPropertyManager 的資料重新更新這一列（名稱 / visible / selected / opacity）。
        """
        item = self.find_item(obj_id)
        if item is None:
            return
